...
/**
 * Class representing the Door opening state.
 */
public class DoorUsingState extends DeviceUsingState {
    // Logger
    private static final Logger log = Logger.getLogger(DoorUsingState.class.getName());

    /**
     * Instantiates a new Door opening state.
     *
     * @param device the device
     */
    public DoorUsingState(Device device) {
        super(device);
        this.ELECTRICITY_CONSUMPTION = 0.5;

        // Logging
        log.info(String.format("Door in room \"%s\" was opened now [%s]",
